                        ...
                        <tbody>
                            @foreach ($cancelledSubscriptions as $cancel)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-{{ $cancel->status == 'cancelled' ? 'danger' : 'secondary' }}">
                                            {{ ucfirst($cancel->status) }}
                                            @if ($cancel->status == 'paused')
                                                {{ '(' . $cancel->pause_start . ' - ' . $cancel->pause_end . ')' }}
                                            @endif
                                        </span>
                                    </td>
                                    <td>
                                        <button class="btn btn-success btn-sm btn-reactivate"
                                            data-id="{{ $cancel->id }}">
                                            Reactivate
                                        </button>

                                        <form id="reactivate-form-{{ $cancel->id }}" style="display: none;"
                                            action="{{ route('subscriptions.reactivate', $cancel->id) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                        </form>
                                    </td>
